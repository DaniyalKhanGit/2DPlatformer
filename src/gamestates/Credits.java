...
public class Credits extends State implements Statemethods {
	...
	private void loadEntities() {
		...
		entitiesList.add(new ShowEntity(getIdleAni(LoadSave.GetSpriteAtlas(LoadSave.PLAYER_ATLAS), 5, 100, 100),
				(int) (Game.GAME_WIDTH * 0.06), (int) (Game.GAME_HEIGHT * 0.7)));
		entitiesList.add(new ShowEntity(getIdleAni(LoadSave.GetSpriteAtlas(LoadSave.SLIME_SPRITE), 11, 16, 32),
				(int) (Game.GAME_WIDTH * 0.2), (int) (Game.GAME_HEIGHT * 0.6)));
		entitiesList.add(new ShowEntity(getIdleAni(LoadSave.GetSpriteAtlas(LoadSave.PINKSTAR_ATLAS), 8, 34, 30),
				(int) (Game.GAME_WIDTH * 0.7), (int) (Game.GAME_HEIGHT * 0.75)));
		entitiesList.add(new ShowEntity(getIdleAni(LoadSave.GetSpriteAtlas(LoadSave.SHARK_ATLAS), 8, 34, 30),
				(int) (Game.GAME_WIDTH * 0.8), (int) (Game.GAME_HEIGHT * 0.8)));
	}

	private BufferedImage[] getIdleAni(BufferedImage atlas, int spritesAmount, int width, int height) {
		BufferedImage[] standingSoldier = new BufferedImage[spritesAmount];
		for (int i = 0; i < spritesAmount; i++)
			standingSoldier[i] = atlas.getSubimage(width * i, 0, width, height);
	...
	private class ShowEntity {
		...
		public void draw(Graphics g) {
			g.drawImage(idleAnimation[aniIndex], x, y, (int) (idleAnimation[aniIndex].getWidth() * 4),
					(int) (idleAnimation[aniIndex].getHeight() * 4), null);
		}
