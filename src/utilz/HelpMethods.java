...
public class HelpMethods {
	...
	public static boolean IsEntityOnPosition(Entity entity, int[][] lvlData, int tileValue) {
        int tileX = (int)(entity.getHitbox().x / Game.TILES_SIZE);
        int tileY = (int)(entity.getHitbox().y / Game.TILES_SIZE);
        if (tileX >= 0 && tileX < lvlData[0].length && 
            tileY >= 0 && tileY < lvlData.length) {
            return lvlData[tileY][tileX] == tileValue;
        }
        return false;
    }
	
	public static boolean IsEntityOnSpikes(Entity entity, int[][] lvlData) {
	    return IsEntityOnPosition(entity, lvlData, SPIKE);
	}
	...
	public static boolean IsSightClear_OLD(int[][] lvlData, Rectangle2D.Float firstHitbox,
			Rectangle2D.Float secondHitbox, int yTile) {
		int firstXTile = (int) (firstHitbox.x / Game.TILES_SIZE);
		int secondXTile = (int) (secondHitbox.x / Game.TILES_SIZE);
